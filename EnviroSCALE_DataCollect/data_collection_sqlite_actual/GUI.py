...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe("paho/test/iotBUET/bulk/")
...
def on_message(client, userdata, msg):
    try:
        ...
        publish("iot.eclipse.org", "bulk", "CO2: " + str(bb[3]) + "\n" + "Temperature: " + str(bb[5]) + "\n" +  "Methane: " + str(bb[1]) + "\n" +  "Humidity: " + str(bb[6]) ,"" , 0, 0, 0)

        publish("iot.eclipse.org", "methane", bb[1], bb[0], 0, 0, 0)
        #publish("iot.eclipse.org", "lpg", bb[2], bb[0], 0, 0, 0)
        publish("iot.eclipse.org","co2", bb[3], bb[0], 0, 0, 0)
        #publish("iot.eclipse.org", "dust", bb[4], bb[0], 0, 0, 0)
        publish("iot.eclipse.org", "temp", bb[5], bb[0], 0, 0, 0)
        publish("iot.eclipse.org", "humidity", bb[6], bb[0], 0, 0, 0)
...
def publish(hostname, event, value,  timestamp, lat, lon, alt, device_id="74da382afd91", prio_class="low", prio_value=10 ):
    ...
    try:
        ...
        topic_phone = "iotBUET/" + device_id + "/" + event
        ...
        if(event=="bulk"):
            pub.single(topic_phone, payload="" + str(value), hostname=hostname, port=1883)
        else:
            pub.single(topic_phone, payload=event + ": " + str(value), hostname=hostname, port=1883)
...
class Dashboard:
    def __init__(self, master, queue, end_command):
        ...
        self.label = Label(master, text="Live readings from PI (ACTUAL)")
